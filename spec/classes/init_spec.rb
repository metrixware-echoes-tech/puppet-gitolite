...
describe 'gitolite', :type => 'class' do
  ...
  describe 'validations' do
    ...
    context 'should validate package_ensure is a valid ensure on package type' do
      let(:params) { { :package_ensure => 'please', :admin_key_content => 'key' } }
      it { should compile.and_raise_error(/parameters are present/) }
    end
    ...
    context 'should ensure allow_local_code when local_code_in_repo' do
      let(:params) { { :local_code_in_repo => true, :allow_local_code => false, :admin_key_content => 'key' } }
      it { should compile.and_raise_error(/must be true to enable/) }
    end
  end # Validations
